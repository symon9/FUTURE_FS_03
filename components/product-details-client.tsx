...
export function ProductDetailsClient({ product, relatedProducts }: { product: Product; relatedProducts: Product[] }) {
  ...
  // GSAP animation for the "Add to Bag" button text
  const handleAddToCart = () => {
    ...
    // Create a GSAP timeline for a clean sequence
    const tl = gsap.timeline();
    tl.to(idleText, { y: '-150%', duration: 0.5, ease: 'power3.in' })
      .fromTo(addingText, { y: '150%' }, { y: '0%', duration: 0.5, ease: 'power3.out' }, '-=0.4');

    // Simulate an API call and reverse the animation
    setTimeout(() => {
      setIsAdding(false);
      gsap.timeline()
  ...
  // GSAP animation for staggering in the gallery images
  useLayoutEffect(() => {
            ...
            <div className="lg:sticky top-24">
                ...
                <div className="mt-8">
                    ...
                    <div className="flex gap-2 mt-4">
                        {sizes.map(size => (
                            <button key={size} onClick={() => setSelectedSize(size)} className={`w-12 h-12 flex items-center justify-center text-sm font-bold border transition-colors duration-200 ${selectedSize === size ? 'bg-white text-black border-white' : 'border-neutral-700 hover:border-white'}`}>{size}</button>
                        ))}
                    </div>
                </div>
                ...
                <button
                    ...
                    disabled={isAdding}
                >
                    <span className="idle-text inline-block">ADD TO BAG</span>
                    <span className="adding-text absolute inline-block translate-y-[150%]">ADDING...</span>
                </button>

                <div className="mt-8">
                    <Accordion title="Description"><ReactMarkdown>{product.description}</ReactMarkdown></Accordion>
                    <Accordion title="Shipping & Returns"><p>Complimentary express shipping on all orders. Please note, delivery times may be longer than usual.</p><p className="mt-4">You can return any order within 14 days of the delivery date.</p></Accordion>
                    <Accordion title="Materials"><p>Composition: 100% Premium Cotton</p><p>Product ID: 612966TIV541070</p></Accordion>
                </div>
            </div>
        </div>
      </div>
            ...
            {relatedProducts.map(item => (
                <Link href={`/product/${item.slug}`} key={item.id} className="group">
                    <div className="aspect-3/4 bg-neutral-900 overflow-hidden">
                        <Image src={item.images[0]} alt={item.name} width={800} height={1067} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105" />
                    </div>
